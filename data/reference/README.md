...
| `derived` | `TRUE` when `count` was computed from the prose (e.g. "Annie + 23 others") rather than stated as a single figure. 8 rows. |
...
exist so that a mismatch with the parser can be attributed before it is called an error.
