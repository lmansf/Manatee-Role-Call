...
pages, Open-Meteo, the USGS gauge, Resend, Google Sheets) could be assumed reachable. Going
...
- GitHub Actions on a schedule. The owner rejected it.
- An always-on small machine or free cloud VM. Deferred, because the personal machine is
  simpler, and every run catches up from the last successful run, so a missed run loses at
  most that day's weather forecast.
